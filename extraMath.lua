...
function module.dist(x1,y1, x2,y2) return ((x2-x1)^2+(y2-y1)^2)^0.5 end
-- Distance between two 3D points:
function module.dist2(x1,y1,z1, x2,y2,z2) return ((x2-x1)^2+(y2-y1)^2+(z2-z1)^2)^0.5 end
...
function module.angle(x1,y1, x2,y2) return math.atan2(y2-y1, x2-x1) end


-- Returns the closest multiple of 'size' (defaulting to 10).
function module.multiple(n, size) size = size or 10 return math.round(n/size)*size end


-- Clamps a number to within a certain range.
function module.clamp(low, n, high) return math.min(math.max(low, n), high) end
...
function module.lerp(a,b,t) return (1-t)*a + t*b end
function module.lerp2(a,b,t) return a+(b-a)*t end

-- Cosine interpolation between two numbers.
function module.cerp(a,b,t) local f=(1-math.cos(t*math.pi))*.5 return a*(1-f)+b*f end
...
function module.sign(n) return n>0 and 1 or n<0 and -1 or 0 end
...
function module.findIntersect(l1p1x,l1p1y, l1p2x,l1p2y, l2p1x,l2p1y, l2p2x,l2p2y, seg1, seg2)
